...
a desirable option. To overcome these problems, several promising dynamic approaches have
...
of a runtime that operates by first invoking a  signal handler, and then by
...
%%added new
A key to effective program repairing is reducing the problem such as by 
targeting a specific domain, or specific data types including strings and then using string
transformation and solving technique for repairing purpose. Singh and Gulwani
\cite{Singh:2012} deal with semantic transformation of the string
...
standard data-type like date, time, and currency. Similarly, Samimi et al. \cite{SamirniSAMTH12}
présent a repairing technique for PHP applications based on string constraint solving.
Gulwani \cite{Gulwani:2011} designed a learning algorithm for learning a string expression that is
consistent with input output examples.

The input output example is generated
