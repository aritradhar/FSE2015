...
branch is a one that does represent erroneous control flow. For example,  a
...
error paths.} Code~\ref{snippet:constraintCollection} shows a string variable
with several constraints as defined by the \code{if} statements on lines
$2$-$5$. The first three constraints are static as all of them can be evaluated
at the compilation time. By analyzing these conditional statements one by one,
\tool\ populates the constraint store data structure as depicted in
Figure~\ref{fig:constraintExample}. \ignore{We define the preferred branch as
the one that does not throw exceptions or error conditions, like
\code{System.err.print()}. In other words, \tool\ only considers the conditional
expressions in the branches that do not involve any exceptions or error paths.}
\ignore{Note that \tool\ also evaluates $OP$ (in
...
where all $x$, $y$ and $z$ are known to be numeric.} As described in
Algorithm~\ref{algo:constraintCollection}, while collecting constraints \tool\
also evaluates composite mathematical expressions, such as $x + y * z$, where
all $x$, $y$ and $z$ are known to be numeric. \note{Please check.}
