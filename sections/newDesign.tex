...
\code{String} objects.
\ignore{The definition of
criticality is solely over the developers. In some cases one would like to
patch the \code{String} objects which are tainted.} \note{Rephrased and compressed.} \tool\, being a highly
configurable tool, allows developers to define their own sets of sources and
sinks, and to exclude specific tainted paths. \ignore{In the default behavior
...
By default, \tool\ makes a safe assumption that any object leaving the
system has a potential to trigger an unintended behavior.
...
\code{String} object is defined as a set of permissible values that can uniquely
...
program to collect constraints on \code{String} objects, such as their
length or prefix. \tool\ invokes the dynamic component if there are constraints
such that the constraint set cannot be evaluated. In
such scenarios it (i) generates a patch that itself dynamically collects
...
exceptions. \tool\ propagates constraints to ensure that the static constraint 
collection works correctly if the conditional statements
involve any variables which are redefined during the collection and can be
calculated statically. \tool\ only collects constraints which are directly
associated to \code{String} objects. Algorithms~\ref{algo:patchingStrategy}
...
Specifically, \tool\ \ignore{iterates over all program code} identifies and analyzes conditional
statements involving string objects \ignore{of the form} such as \code{if (st.length() == $5$)}.
It considers only those constraints that the object must satisfy to ensure
that the control flows through one of the \textit{normal} branches of the conditional.
A normal branch is a one that does represent erroneous control flow. For example,  a branch
that encounters a statement \code{System.err.print()} would represent an erroneous control flow.
\ignore{In other words, \tool\ only
considers the conditional expressions in the branches that do not involve any
exceptions or error paths.}
Code~\ref{snippet:constraintCollection} shows a string variable with several constraints
as defined by the \code{if} statements on lines $2$-$5$.
The first three constraints are static as
all of them can be evaluated at the compilation time. By analyzing these conditional
statements one by one, \tool\ populates the constraint store data structure as
depicted in Figure~\ref{fig:constraintExample}.
\ignore{We define the preferred branch as the one that does not throw exceptions
or error conditions, like \code{System.err.print()}. In other words, \tool\ only
considers the conditional expressions in the branches that do not involve any
exceptions or error paths.} \ignore{Note that \tool\ also evaluates $OP$ (in
...
As described in Algorithm~\ref{algo:constraintCollection}, while collecting constraints \tool\ also evaluates composite mathematical expressions, such as $x + y * z$, 
where all $x$, $y$ and $z$ are known to be numeric. \note{Please check.}
...
completely collected and evaluated statically, \tool\
instruments the code with statements to dynamically collect constraint
information, augments them with previously known static constraints, and evaluates
...
consideration. Code~\ref{snippet:exCode2} depicts the example in Code~\ref{snippet:constraintCollection}
after it is instrumented by \tool. When the constraints in the source are complex, \tool\ relies on its base framework
to simplify them and then evaluates only the ones that are related to strings.
\note{I removed the complicated example. Please check.}
\ignore{One complex example of
dynamic constraint collection and evaluation is \code{x > 10} and
\code{s.length() < x * x \&\& x * x > 10}. In such case, \tool\ with the help
of \code{jimple} intermediate representation (IR) breaks down the second conjugate
constraint to two simple ones. As \code{x > 10} and \code{x * x > 10} do not
involve any \code{String} object, \tool\ will ignore them and evaluate
\code{s.length() < x * x } dynamically when the value of x is available just
