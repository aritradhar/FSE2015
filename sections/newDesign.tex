...
depicted in Figure~\ref{fig:constraintExample}. \ignore{We define the preferred
branch as the one that does not throw exceptions or error conditions, like
...
where all $x$, $y$ and $z$ are known to be numeric.} When collecting
...
\note{
%%I removed the complicated example. Please
check.}
\ignore{One complex example of dynamic constraint collection and evaluation is
\code{x > 10} and \code{s.length() < x * x \&\& x * x > 10}. In such case,
\tool\ with the help of \code{jimple} intermediate representation (IR) breaks
down the second conjugate constraint to two simple ones. As \code{x > 10} and
\code{x * x > 10} do not involve any \code{String} object, \tool\ will ignore
them and evaluate \code{s.length() < x * x } dynamically when the value of x is
available just before the conditional statement.}
