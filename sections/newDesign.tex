...
\code{String} objects. \note {The definition of
criticality is solely over the developers. In some cases one would like to
patch the \code{String} objects which are tainted. \tool\ is highly
...
Figure~\ref{fig:constraint}.\note{We named this data structure \emph{Constraint
store}}.
...
exceptions. \note{We leverage constraint propagation technique to make
sure static constraint collection works properly if the conditional statements
involve any variables which re defined or reassigned before and can be
...
We define the preferred branch as the one that does not throw exceptions or error conditions, like \code{System.err.print()}. In other words, \tool\ only
...
\item \lstset{language=Java, caption=Example of parameter tweaking.,
...
limited number of \code{String} characteristics. This may create complication
if the program contains very complex constraints. But  we tested \tool\ on
several library APIs described in Section~\ref{sec:results} and multiple
synthetic test cases composed by us and found no problem.}
