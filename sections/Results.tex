...
reported. Thus, the developer's fix was significantly larger than \tool's patch.
 ...
 \item We notice low PQI (between $0.5$ and $0.79$) for four of the patched
...
If there is exist some developers' exception handling mechanism in the method or
in any methods upper in the call chain, the patching module will skip that as it
may change natural program flow. In some cases we have seen this where the buggy
string handling statement is inside the try block and handled by wrong exception
type (often \code{Exception} class which is the base type). In such cases to
evaluate the effectiveness of our patching technique, we disabled the call graph
analysis phase and do a force patch in the methods. In such cases the
developer's \code{catch} block couldn't handle that and we deviated from natural
flow. In such cases PFCI = n, where n is the number of developer's exception
handling we ignore for force update.
