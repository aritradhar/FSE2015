...
\centering
\scriptsize
\begin{tabular}{|l|c|l|r|r||r|r|r|r||r|r|r|c|}
...
in unpatched & $\mathcal{IC}_{NO}$ & Instrumentation w/o optimization (recall
\xref{subsec:optimizations})\\

$\mathcal{N}_{Unit}$& \# \code{Units} analyzed & $\mathcal{S}_{P}$ & \#
successful tests in patched version & $\mathcal{IC}_{WO}$ & Instrumentation w/
optimization (recall \xref{subsec:optimizations})\\

$\mathcal{S}_{U}$ & \# successful tests in unpatched & $\mathcal{F}_{P}$ & \#
fail tests in patched version & $\mathcal{RS}_{CE}$ & Cascaded exception
exists\\
...
\item \textbf{Effectiveness of the patch}: A software patch is considered if it
does not violate any existing test case from the test suite. Thus, we
determined the effectiveness of \tool\ generated patches by running them
against the the benchmark's existing test suites 


Table~\ref{tab:results} lists $30$
real-world bugs mined from bug repositories of popular open-source libraries. We
wrote a driver program to recreate the bug, and then applied \tool\ to patch it.
We observed that in all cases, \tool\ successfully patched the offending class
file in the concerned library.
...
bug. We define \textbf{Patch Quality Index (PQI)} as a measure of the
precision of the patch.
$$PQI = \frac{\#~Constraints_{\tool}}{\#~Constraints_{Developer}}$$
% * \frac{\#~LOC_{Developer}}{\#~LOC_{\tool}} *
% \frac{Output_{\tool}}{Output_{Developer}}$$

Specifically, PQI compares the similarities in constraints in \tool's patch
...
fix, the PQI will be lesser. Thus, a higher value of PQI, \ie\ closer to $1$, is
preferred.

PQI can be computed automatically, since \tool\ already generates a list
of constraints, and static program analysis of the developer's patch can
provided the same. However, in our current implementation, we manually compute
the constraints for the developer's fix. Table~\ref{tab:results} lists the PQI
for the benchmarks in our set.
...
\note{redo the items below based on the PQI observed.}
\begin{mybullet}
