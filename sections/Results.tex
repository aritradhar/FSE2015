...
& $\checkmark$\\
%% 0.96875
\code{SOAP} & \cite{SOAP130} & Major &$5.0K$ & $165$ & $0.97$ & $0$  & $32$ &
$5$
...
instance where the concerned bug~\ref{} was still unpatched. In such cases, we
looked for the potential or suggested patches in comments and discussion forums,
and compared \tool's patch with them.
...
reported. Thus, the developer's fix was significantly larger than \tool's patch.
 ...
 \item For couple of the applications we experienced low PQI index which is as
low as .5 which shows that the quality of the auto generated patch was surpassed
significantly by the develops' one. The only explanation of such scenario is the
cascaded exception we got after executing the patched library. Though the
...
driver string to some place else. For the auto generated patch, it is not always
guaranteed that the patch will make sure all the required specification. In such
cases, we considered our patches have lower quality then the original ones. But
for all of the cascaded exceptions were checked exception and it is expected
that the application developers would take care of those.
...
\centering
\begin{tabular}{ccc}
\subfloat[Prob. of false alarms with variation in $\tau$ and
flows.\label{fig:perf:callgraph}]
{\includegraphics[width=.3\linewidth]{images/CgSizeVsTime.pdf}}
\hfill
&
\subfloat[Prob. of lack of genuine alarms vs $\tau$ and loss
rate.\label{fig:perf:constraints}]
{\includegraphics[width=.3\linewidth]{images/ConstraintsVsTime.pdf}
}
\hfill
&
\subfloat[Comparison of ping latencies with varying
hosts.\label{figure:perf:strings}]
...
\begin{mylist}
 ...
 \item \textbf{Execution overhead}: We selected randomly selected and patched
$5$ libraries (\todo{names}) from Table~\ref{tab:results} to determine the
execution overhead of the patched class files. We observed that for execution
runs of $100K$, the $5$ benchmarks reported an average overhead of $x\%$, with
the maximum absolute overhead being $x\mu$s, which is imperceptible at human
...
\begin{mylist}
 ...
 \item The bug~\ref{ARIES1204} as reported in the repository for Apache Aries
...
framework to ensure conformance.
 ...
 \item Commons Math had a bug related to incorrect formatting of the input
string, while the library was throwing a string exception which is completely
irrelevant.
According to the API documentation, the method supposed to throw a very specific
type of exception (\code{NumberFormatException})which contains the information
of the malformed string. In such cases we see if the patch version also throws
that specific exception which is the correct behavior.
...
patch the bugs. In order to measure the effectiveness, we turned off the call
graph analysis module and thus forced \tool\ to generate a patch for the bug so
as to compare it with the developer's fix.
