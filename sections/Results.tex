...
tests in patched version before force patching & $\mathcal{IC}_{WO}$ &
...
Index & $\mathcal{F'}_p$ & \#fail test in patched version after force patching\\
...
with all optimizations enabled successfully patched the offending bug in $28$ of
the $30$ library benchmarks in our data set (see column $\mathcal{F}_{P}$ in
Table~\ref{tab:results}), thereby achieving an effectiveness of over $93$\%.

Note that the patched versions of \code{Commons CLI2.x} and \code{Eclipse AJ
Weaver} benchmarks fail a total of three test cases. We observed that for both
these benchmarks, a cascaded exception was thrown that resulted in the failure
of the test case.
