...
However, there is always a tradeoff between the precision and the scalability that a
static program analysis has to live with. In order to achieve a
...
As a result, successful execution of a real application can never be guaranteed, and unexpected failures
may happen. These failures often result in runtime exceptions being thrown by
the applications that are generally not handled and the program crashes.

%The cost of these runtime
