...
\code{String} APIs are commonly used in \java\ applications and libraries.
...
or force the \java\ \code{String} API by tweaking their parameters to generate
...
line 10 when the method is called with parameter \code{"/foo.xml"}.  Here, the
...
Code~\ref{snippet:exampleRepairing2} presented the patch automatically
generated by \tool\
using our approach. \tool\ first identifies that statement 10 in
Code~\ref{snippet:exampleRepairing2}
can be vulnerable to a failure and might throw \code{IndexOutOfBoundsException}
...
\xref{sec:architecture}. As a result it replaces statement 10 with the
...
which computes legally correct bounds required for \code{substring} method that
...
variable \code{temp} is regenerated after legally correct adjustments of the
indices.
