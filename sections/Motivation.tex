...
or force the \java\code{String} API by tweaking their parameters to generate the data, iv) optimises the
...
regenerate string objects either by tweaking \java\code{String} API used in
...
Library , label = snippet:exampleRepairing1, basicstyle=\scriptsize}
\begin{figure}[t]
\begin{lstlisting}
...
line 10 when the method is called with parameter \code{"/foo.xml"}.  Here, the value of \code{prefix} as
...
in Apache Commons Library bug, label = snippet:exampleRepairing2, firstnumber
=10, basicstyle = \scriptsize}
\begin{figure}[t]
\begin{lstlisting}
String temp = null;
try {
...
Code Snippet~\ref{snippet:exampleRepairing2} presented the patch automatically generated by \texttt{Clotho}
using our approach. \texttt{Clotho} first identifies that statement 10 in Code Snippet~\ref{snippet:exampleRepairing2}
can be vulnerable to a failure and might throw \code{IndexOutOfBoundsException} which is not handled in the program.
...
Snippet~\ref{snippet:exampleRepairing2}. The statement 17 shows two methods namely \code{getStart} and \code{getEnd}
which computes legally correct bounds required for \code{substring} method that regenerates string referenced by variable \code{temp}. 
...
variable \code{temp} is regenerated after legally correct adjustments of the indices.
